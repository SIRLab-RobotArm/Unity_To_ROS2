...
class DualIKSolver(Node):
    def __init__(self):
        super().__init__("dual_ik_solver")

        # LEFT arm pose subscriber
    ...
    # Common IK request
    # ==========================
    def send_ik_request(self, pose_msg, arm):
        req = GetPositionIK.Request()
        req.ik_request.group_name = "arm"   # ← ★ 둘 다 같은 group
    ...
    # IK result callback
    # ==========================
    def process_ik_result(self, future, arm):
        try:
            res = future.result()
        ...
        self.get_logger().info(f"[{arm.upper()}] IK returned: {sol.name}")
